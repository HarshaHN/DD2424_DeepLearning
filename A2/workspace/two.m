...
function two
    ...
    GDparams.n_batch = 100; GDparams.n_epochs = 1;
...
function [ gradTheta] = CompGradients(X, Y, FP, theta, lambda)
    ...
    %Initialize
    LossW = 0; Lossb = 0; gW1 =0; gW2 =0; gb1 =0; gb2 =0;
    
    %Update loop
    N = size(X, 2); H = FP{1}; P = FP{2}; m = size(H,1);
    for i = 1:N % ith image
        ...
        %mxN
        ng = g*theta{1,2}; %1xK*K*m = 1xm
        nng = ng*(H(:,i)>0)*eye(m); %1xm*mxm = 1xm
    ...
    for r=1:2
        for c=1:2
            %gradTheta{1,1} = (1./size(X, 2)) * (g{}) + lambda .*2*theta.W1;
            gradTheta{1,2} = (1./size(X, 2)) * (LossW) + lambda .*2*theta.W2;
            gradTheta{2,1} = (1./size(X, 2)) * (Lossb);
            gradTheta{2,2} = (1./size(X, 2)) * (Lossb);    
        end
    end
end
